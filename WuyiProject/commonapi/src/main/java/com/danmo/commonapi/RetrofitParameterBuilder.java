...
public class RetrofitParameterBuilder {
    ...
    public RetrofitParameterBuilder addParameter(String key, Object o) {
        if (o instanceof String) {
            RequestBody requestBody=RequestBody.create(MediaType.parse("text/plant"),(String) o);
            ...
            for(File s:(ArrayList<File>)o){
                RequestBody body=RequestBody.create(MediaType.parse("multipart/form-data;charset=UTF-8"),(File)s);
                params.put(key + "\"; filename=\"" + ((File) s).getName() + "", body);
            }
        }
        return this;
    }

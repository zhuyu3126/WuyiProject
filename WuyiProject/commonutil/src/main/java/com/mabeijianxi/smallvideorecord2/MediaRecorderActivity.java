 ...
 * 视频录制
 */
public class MediaRecorderActivity extends Activity implements MediaRecorderBase.OnErrorListener, OnClickListener, MediaRecorderBase.OnPreparedListener, MediaRecorderBase.OnEncodeListener {

    public static final int REQ_CODE_START_RECORD = 999;
    ...
    public static void goSmallVideoRecorder(Activity context, String overGOActivityName, MediaRecorderConfig mediaRecorderConfig) {
        if (TextUtils.isEmpty(overGOActivityName)) {
            context.startActivityForResult(new Intent(context, MediaRecorderActivity.class).putExtra(OVER_ACTIVITY_NAME, overGOActivityName)
                    .putExtra(MEDIA_RECORDER_CONFIG_KEY, mediaRecorderConfig), REQ_CODE_START_RECORD);
        } else {
    ...
    public static void goSmallVideoRecorder(Activity context, MediaRecorderConfig mediaRecorderConfig, int code) {
        context.startActivityForResult(new Intent(context, MediaRecorderActivity.class).putExtra(OVER_ACTIVITY_NAME, "")
                .putExtra(MEDIA_RECORDER_CONFIG_KEY, mediaRecorderConfig), code);
    }
    ...
    private void initMediaRecorder() {
        ...
        mMediaObject = mMediaRecorder.setOutputDirectory(key, JianXiCamera.getVideoCachePath() + key);
...
/*                    if (mPressedStatus) {
    ...
    public void onEncodeComplete() {
        ...
        Intent intent = null;
        try {
            if (TextUtils.isEmpty(getIntent().getStringExtra(OVER_ACTIVITY_NAME))) {
                intent = new Intent();
            } else {

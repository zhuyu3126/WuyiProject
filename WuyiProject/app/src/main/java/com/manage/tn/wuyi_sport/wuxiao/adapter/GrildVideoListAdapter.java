...
public class GrildVideoListAdapter extends SingleTypeAdapter<VideoItem> {
    List<VideoItem> videoItemsAll;

    public GrildVideoListAdapter(@NonNull Context context, List<VideoItem> all) {
    ...
    public void convert(int position, RecyclerViewHolder holder, VideoItem bean) {
        ...
        Glide.with(mContext)
                ...
                .apply(options)
                .into(item_image);
        holder.setText(R.id.item_pub, DateUtil.isEmpty(bean.getPublishTime())?"发布于 00-00-00":"发布于 "+bean.getPublishTime().substring(0,10));
        holder.setText(R.id.item_vc,"浏览量 "+bean.getReadNum());
        holder.setText(R.id.videoTitle,DateUtil.isEmpty(bean.getName())?"":bean.getName());
